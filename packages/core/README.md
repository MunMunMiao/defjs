...
Endpoint definitions are stricter: `defineRequest`, `defineEventStream`, and `defineWebSocket` require an `input` struct whenever `build` is provided. Endpoints without an `input` struct can still omit `build`.
