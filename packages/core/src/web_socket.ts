...
export interface WebSocketRef<TIncoming = unknown, TOutgoing = never> extends PromiseLike<SocketAwaitResult<TIncoming, TOutgoing>> {
  (config?: UseWebSocketConfig<TIncoming, TOutgoing>): WebSocketRef<TIncoming, TOutgoing>
...
export interface WebSocketEndpoint<
  ...
  TIncoming extends SocketSchemas = SocketSchemas,
  TOutgoing extends SocketSchemas | undefined = undefined,
> extends WebSocketDefinition<TInput, TIncoming, TOutgoing> {
  readonly kind: 'web-socket'
  readonly use: UseWebSocketEndpointFn<TInput, TIncoming, TOutgoing>
}

type SocketRefConfig = {
  abort?: AbortSignal
  applied: boolean
  beforeConnect?: WebSocketBeforeConnect
  heartbeat?: WebSocketHeartbeatConfig<any, any>
  protocols?: readonly string[]
  queue?: WebSocketQueueConfig
  reconnect?: WebSocketReconnectConfig
  timeout?: number
...
export function defineWebSocket<
  ...
  TIncoming extends SocketSchemas,
  TOutgoing extends SocketSchemas | undefined = undefined,
>(definition: WebSocketDefinition<TInput, TIncoming, TOutgoing>): WebSocketEndpoint<TInput, TIncoming, TOutgoing> {
  const endpoint = {
    ...definition,
    kind: 'web-socket' as const,
  } as WebSocketEndpoint<TInput, TIncoming, TOutgoing>

  Object.defineProperty(endpoint, 'use', {
    enumerable: true,
    value: createUseWebSocket(endpoint),
  })

  return endpoint
}

export function createUseWebSocket<
  TInput extends AnyCompatibleSchema | undefined,
  TIncoming extends SocketSchemas,
  TOutgoing extends SocketSchemas | undefined,
>(endpoint: WebSocketEndpoint<TInput, TIncoming, TOutgoing>): UseWebSocketEndpointFn<TInput, TIncoming, TOutgoing> {
  return ((input?: EndpointInput<TInput>) =>
    createWebSocketRef(endpoint, input, {
      applied: false,
    })) as UseWebSocketEndpointFn<TInput, TIncoming, TOutgoing>
}
...
>(
  ...
  const applyConfig = ((nextConfig?: UseWebSocketConfig<WebSocketIncomingData<TIncoming>, WebSocketOutgoingData<TOutgoing>>) => {
    if (config.applied) {
      throw new Error('WebSocket config can only be applied once')
    }

    return createWebSocketRef(endpoint, input, {
      ...nextConfig,
      applied: true,
    })
  }) as WebSocketRef<WebSocketIncomingData<TIncoming>, WebSocketOutgoingData<TOutgoing>>
  ...
  applyConfig.close = (code?: number, reason?: string) => {
    controller.abort(createManualSocketCloseReason(code, reason))
    state.socket?.close(code, reason)
  }

  applyConfig.onRuntimeError = listener => {
  ...
  applyConfig.onStateChange = listener => {
...
function createManualSocketCloseReason(code?: number, reason?: string): ManualSocketCloseReason {
  return {
    code,
    kind: 'manual-web-socket-close',
    reason,
