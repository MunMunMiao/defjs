...
```ts
import { type ClientXSRFOptions, type XSRFTokenProvider, withXSRF } from './index'
...
- [ ] **Step 1: 在 `fetch.spec.ts` 写失败用例，锁定 transport 行为矩阵**

```ts
test('should inject xsrf header for mutating same-origin browser requests', () => {
  vi.stubGlobal('document', { cookie: 'XSRF-TOKEN=abc123; theme=dark' })
...
test.each(['GET', 'HEAD'] as const)('should not inject xsrf header for %s requests', (method) => {
  vi.stubGlobal('document', { cookie: 'XSRF-TOKEN=abc123' })
  vi.stubGlobal('location', new URL('https://example.com'))
...
test('should not inject xsrf header for cross-origin browser requests', () => {
  vi.stubGlobal('document', { cookie: 'XSRF-TOKEN=abc123' })
...
test('should prefer provider over browser cookie and keep withCredentials independent', () => {
  vi.stubGlobal('document', { cookie: 'XSRF-TOKEN=cookie-token' })
  vi.stubGlobal('location', new URL('https://example.com'))
...
- [ ] **Step 2: 在 `http.browser.spec.ts` 写失败用例，锁定真实浏览器里的 cookie 读取与 same-origin 注入**
...
- [ ] **Step 3: 运行 transport 单测与 browser 回归测试，确认它们先失败**

Run:

```bash
cd packages/core && bun ../../node_modules/vitest/vitest.mjs run --config vitest.config.bun.ts src/http/transport/fetch.spec.ts
...
- [ ] **Step 4: 以最小改动实现 transport XSRF 逻辑**
...
- [ ] **Step 5: 重新运行 transport 单测与 browser 回归测试，确认通过**
...
- [ ] **Step 6: 提交 transport 与浏览器回归改动**

```bash
git add packages/core/src/http/transport/fetch.ts packages/core/src/http/transport/fetch.spec.ts packages/core/src/http/http.browser.spec.ts
...
```bash
cd packages/core && bun ../../node_modules/vitest/vitest.mjs run --config vitest.config.bun.ts src/client/client.spec.ts src/http/http.spec.ts src/http/transport/fetch.spec.ts
```

Expected:

- PASS
- client 配置、request 传播与 transport 行为三层一起通过
...
- 规格里的 public API、runtime 行为矩阵、内部数据流、same-origin 固定策略、`withCredentials` 独立性、同步 provider 约束、测试矩阵都已分别映射到 Task 1–4。
