...
| `makeHttpContext()`                              | 建立空白上下文                                      |
| `makeHttpContext(entries)`                       | 從 `[token, value]` 陣列建立                        |
| `makeHttpContext(otherContext)`                  | 複製另一個上下文                                    |
| `mergeHttpContexts(primary, secondary)`          | 合併兩個上下文；secondary 對同一 Token 覆寫 primary |
...
| `webSocket.queue`         | 發送隊列：最大容量、溢位策略                                     |
