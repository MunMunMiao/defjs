...
تقدم هذه الصفحة أمثلة جاهزة للنسخ واللصق لأكثر حالات الاستخدام شيوعًا.

## REST CRUD

### تعريف المخططات ونقاط النهاية
...
## ورقة مرجعية للواجهة البرمجية

| Export                                                                                      | الاستخدام النموذجي         |
| ------------------------------------------------------------------------------------------- | -------------------------- |
| `createClient(...options)`                                                                  | إنشاء نسخة عميل            |
| `withEndpoint(url)`                                                                         | تعيين عنوان URL الأساسي    |
| `withInterceptors(...interceptors)`                                                         | تسجيل الاعتراضات           |
| `defineRequest({ method, path, input?, build?, output? })`                                  | تعريف نقطة نهاية HTTP      |
| `defineEventStream({ path, events, input?, build? })`                                       | تعريف نقطة نهاية SSE       |
| `defineWebSocket({ path, incoming, outgoing?, input?, build? })`                            | تعريف نقطة نهاية WebSocket |
| `struct.object(shape)`                                                                      | مخطط كائن                  |
| `struct.string()` / `struct.number()` / `struct.boolean()`                                  | مخططات أولية               |
| `struct.array(item)`                                                                        | مخطط مصفوفة                |
| `struct.enum(values)`                                                                       | مخطط تعداد                 |
| `tag.uri()` / `tag.json()` / `tag.query()` / `tag.header()`                                 | وسوم الحقول                |
| `createHttpInterceptor(fn)` / `createSSEInterceptor(fn)` / `createWebSocketInterceptor(fn)` | إنشاء اعتراضات             |
| `basicAuthHttpInterceptor(fn)` / `basicAuthSSEInterceptor(fn)`                              | Basic Auth مدمج            |

## ما التالي

- [العميل →](/core/client) — إنشاء العميل واستخدام `execute`
- [الأوامر →](/core/commands) — تعريفات الأوامر وقواعد اختيارية المدخلات
