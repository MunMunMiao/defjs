...
`@defjs/core` 的所有執行結果都以 `[error, result, response]` 三元組回傳。`error` 是 `RequestError`：一個以 `kind` 與 `code` 區分的可辨識聯合型別。建議以 `kind` 與 `code` 進行分支，而非字串比對。

## RequestError 結構

`RequestError` 是三種錯誤型別的聯合：
...
`data` 型別由該狀態碼對應的 `output` 結構描述推導。例如 `output: { 404: notFoundStruct }` 會將 `error.data` 窄化為 `notFoundStruct` 的推導型別。
