...
`output` 驅動 TypeScript 型別推導。`Client.execute()` 回傳的 `HttpAwaitResult` 會自動區分 2xx 成功資料與非 2xx 錯誤資料。
...
- `RequestSuccessData<TOutput>`：從 `output` 提取所有 2xx 結構描述輸出型別。若無 2xx 對應，推導為 `unknown`。
- `RequestErrorData<TOutput>`：從 `output` 提取所有非 2xx 結構描述輸出型別。若無非 2xx 對應，推導為 `unknown`。
