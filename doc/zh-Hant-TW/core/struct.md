...
`@defjs/core` 提供輕量級的 struct 外觀，用於宣告結構描述、驗證輸入與推導型別。設計意圖參考 Go 的 `encoding/json`：零值兜底、接受部分輸入、穩定且可預測的執行階段行為。
...
`struct.Infer<T>` 提取結構描述的輸出型別。這是你唯一需要掌握型別層級輔助函式。
...
與 `isObjectStruct` 搭配，可在內省前進行安全型別檢查：
