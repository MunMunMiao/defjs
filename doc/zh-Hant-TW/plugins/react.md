...
這個 fragment 假設 `recordRealtimeFailure` 是應用程式的 telemetry function。它刻意消費 `session.receive`；讓這個無界 incoming queue 完全沒人讀取，不是有效的資源管理方式。SSE handle 也要套用相同的 startup 與 cleanup 原則。
