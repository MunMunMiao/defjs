...
`@defjs/vue` 將 `@defjs/core` 整合為 Vue 3 外掛程式程式。它提供 `provideClient` 在應用程式層級註冊 `Client` 實例，以及 `injectClient` 在元件或 Composition API 中存取該實例。
...
`provideClient` 回傳標準 Vue 外掛程式程式。內部使用 `app.provide()` 將 `Client` 實例注入應用程式脈絡。所有子元件皆可透過 `injectClient()` 存取。
...
使用 `withInterceptors` 註冊工廠函式陣列。每個工廠在外掛程式程式安裝時執行，回傳的攔截器實例會註冊到 Client。
...
- [WebSocket 檔案](/core/web-socket) — WebSocket 連線與訊息型別

## API 參考

### `provideClient(...feature: ClientOption[]): Plugin`

建立 Vue 外掛程式程式。安裝時，透過 `createClient(...)` 建構 `Client` 實例，並以 `HTTP_CLIENT` 作為 Injection Key 提供至應用程式脈絡。
...
設定攔截器。每個工廠在外掛程式程式安裝時執行，回傳的攔截器依註冊順序形成洋蔥模型呼叫鏈。
