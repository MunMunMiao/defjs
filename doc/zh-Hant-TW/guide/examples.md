...
重連在這裡明確啟用。Endpoint 會限制 incoming/outgoing 容量，由單一 iterator 消費邏輯 session，並在所有離開路徑關閉它。
...
請傳入原始 placeholder 值。Core 在替換 path 時會對每個值恰好編碼一次；不要預先編碼，否則 `%` 會被再次編碼。這個 recipe 也不記錄最後產生的 URL 或 payload。
