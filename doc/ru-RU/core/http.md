...
Значения плейсхолдеров вставляются без кодирования сегмента пути. Ограничьте формат идентификаторов или вызовите `encodeURIComponent` для одного недоверенного сегмента до создания команды. Незакодированный слеш или сегмент с точками может изменить итоговый путь, а вставленный `?` или `#` приведёт к отклонению пути эндпоинта.
...
Если `output` объявлен, а `responseType` не указан, ответ по умолчанию разбирается как `json`. Явные режимы: `json`, `text`, `blob` и `arraybuffer`. Затем выбранный Struct выполняет структурное декодирование. Если `output` отсутствует, данные результата равны `undefined`, а у возвращённой обёртки ответа `body: null`.

### Текущий дефект с некорректным JSON

::: danger Некорректный JSON может выглядеть как успешный результат
Текущая граница Fetch записывает ошибку разбора JSON в `HttpResponse.error`, а тело оставляет равным `null`. Выполнение HTTP-команды не проверяет эту ошибку перед применением Struct из `output`. Если выбранный Struct не допускает `null`, это значение может декодироваться в его нулевое значение, поэтому некорректное тело 2xx сейчас способно вернуть `[null, zeroValue, response]`.

Не считайте успешный результат с нулевыми значениями доказательством того, что сервер прислал корректный JSON. Нужны исправление реализации и регрессионный тест; документация здесь только предупреждает о дефекте.
:::
...
При успехе `response` — обёртка Defjs `SettledResponse`, тело которой соответствует `data`. При ошибке доступность ответа зависит от того, насколько далеко дошло выполнение. Полная классификация приведена в разделе [«Ошибки»](/ru-RU/core/errors).
...
Токен пытаются добавить только для `POST`, `PUT`, `PATCH` и `DELETE`. Уже существующий настроенный заголовок сохраняется. В браузере cookie ищется только для запросов к тому же origin. Вне браузера передайте синхронный `tokenProvider`; он имеет приоритет перед чтением cookie.
...
- [Struct](/ru-RU/core/struct) — структурное декодирование с нулевыми значениями.
