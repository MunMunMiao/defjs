...
エンドポイント定義は、安定した通信契約を記述します。キャンセル、タイムアウト、ハートビート、再接続、キューの選択は、その処理を所有する実行側の責務です。

HTTP と SSE は実行時にキャンセル設定を受け取ります。WebSocket も接続ごとの `beforeConnect`、ハートビート、再接続、プロトコル、送信キュー設定を受け取ります。トランスポートが対応している項目については、クライアントオプションで再利用可能なデフォルト値を設定できます。
...
SSE の `onInvalidEvent` は、破棄されたイベントを観測します。オブザーバーが例外を送出した場合や、返した Promise が reject された場合もストリームは終了しません。ただし、非同期オブザーバーは完了まで待機されるため、後続メッセージの処理を遅らせることがあります。

WebSocket の状態リスナーとランタイムエラーリスナーもオブザーバーですが、現在の実装はこれらを直接呼び出します。同期かつ例外を送出しない小さな処理にしてください。リスナーの例外はライフサイクル処理を中断する可能性があり、制御フローとしてはサポートされません。
