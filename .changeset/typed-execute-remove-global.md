...
- Per-request options (`signal`, `timeout`, `context`, `onDownloadProgress`, `heartbeat`, `reconnect`, etc.) are now passed as the second argument to `client.execute(command, options)` instead of being attached to the command builder call.
